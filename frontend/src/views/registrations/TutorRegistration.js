...
export function TutorRegistration() {
  ...
  const TutorRegisterrequest = (name, email, password, schoolid) => async (e) => {
    let url = `${API_URL}/tutor/self/register`
    e.preventDefault();
    let query = {
      name: name,
      email: email,
    ...
    var trstatus = (data.status);
    if (trstatus === 0 ){
      history.push("/tutor/dashboard")
    }
    else{
      history.push("/register/tutor")
    }
  };
  ...
  switch (type) {
    ...
    case "tutor":
      ...
      formSubmittionHandler = TutorRegisterrequest(name, email, password, schoolid);
      break;
